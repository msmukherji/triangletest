...
# RUN THE ROLLS
	...
	# HAVE EVERYONE ROLL ONCE, ADD THEIR ROUND SCORE TO THEIR TOTAL SCORE (value)
 		players_and_scores.each do |key, value|
 			...
 			#the line above updates the variable 'value', but not the value in the hash-- why?
 			#the value of 'value' isn't staying updated when it goes through the loop again, it reverts to 0 each time.
...
# 			again = gets.chomp
# 			break if again == "n" else choose_game
# 			#why does y take you to play_hog rather than choose_game? and only if no one scores anything
			#end
puts "\n\nWould you like to play again? (y/n)\n\n"
again = gets.chomp
